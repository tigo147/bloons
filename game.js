...
function updateWave(dt) {
  ...
  while (state.activeWave.spawned < state.activeWave.total && state.spawnTimer >= state.activeWave.interval) {
    ...
    const mod = state.activeWave.spawned % 6;
    const kind = mod >= 4 ? "green" : mod >= 2 ? "blue" : "red";
    spawnBloon(kind);
...
function updateUI() {
  ...
  const remainingFromWave = state.activeWave ? state.activeWave.total - state.activeWave.spawned : 0;
  ui.remaining.textContent = `${state.bloons.length + remainingFromWave}`;
